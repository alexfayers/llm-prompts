...
- **A local check passing is not proof CI will pass if the local environment has diverged from what CI actually provisions** (a long-lived local database with accumulated manual state vs. CI's freshly-provisioned one, a different base image, a cache populated by earlier manual testing, etc.). Before pushing a schema/infra-sensitive change, verify against a freshly-provisioned instance matching CI's, not just the persistent local one - a migration or config change can pass every local check because the local environment already has some prerequisite state that CI's clean provisioning never applies. This is distinct from the stale-baseline point above: that one is about an incremental run skipping unchanged inputs, this one is about the local *environment itself* carrying state a clean CI run never creates.
