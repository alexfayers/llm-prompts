...
If you notice an unrelated issue while working on a change and there is a TODO.md file in the place that you're working, add that issue to the TODO.md file with enough context that it can be picked up and resolved in the future.
...
3. **Add it as a TODO** (in TODO.md or memory) so it is not forgotten.
...
You can and should also store TODOs in memory, but working through file-based TODOs is preferable.

NOTE: You are allowed to write to files in {{PLAN_MODE}} for this use case.