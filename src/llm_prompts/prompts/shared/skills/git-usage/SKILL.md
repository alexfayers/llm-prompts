...
- **A local commit not yet on the upstream remote is not final - it is in-progress work, and is fair game to amend, reword, squash, reorder, or drop.** Do not assume an unpushed commit is off-limits because it looks complete or "belongs" to someone else - in a single-user repo it is almost always leftover work-in-progress from an earlier session, not a finished, reviewed change. Check with `git -P log --oneline @{u}..HEAD`: anything listed there can be freely rewritten to accomplish the current task. This is the general principle behind the amend guidance below and the `git-tidy` skill - the approved-review exception further below is the one case where an unpushed commit is still off-limits.
...
- **Never resolve a conflict with `checkout --ours`/`--theirs` - name the source ref explicitly instead.** Their meaning flips between the two commands: during `git rebase <base>`, `--ours` is the base being rebased onto; during `git merge <branch>`, `--ours` is the current checked-out branch - the opposite mental model. Applying rebase intuition to a merge (or vice versa) silently resolves to the wrong side with no error. Use `git checkout <ref> -- <file>` (e.g. `git checkout origin/main -- <file>`) instead, which is unambiguous regardless of merge vs rebase context.
