...
def validate(data: dict) -> dict:
    ...
    errors: list[str] = []
    categories = [
        c for c in QUALITY + CONFIDENCE if not (c == "testability" and skip_testability)
    ]
    ...
    for cat in categories:
        if cat not in scores:
            ...
            errors.append(
                f"'{cat}' = {scores[cat]} needs evidence (cite file/pattern/finding)"
            )
    ...
    below = [
        {"category": c, "score": scores[c]} for c in categories if scores[c] < THRESHOLD
    ]
    ...
    return {
        ...
        "gate_message": (
            f"PASS: quality={q_avg:.1f}, confidence={c_avg:.1f}."
            if passed
