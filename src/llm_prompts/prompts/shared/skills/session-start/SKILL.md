...
2. `search_all_projects(query="task", status="in-progress")` and `search_all_projects(query="task", status="planned")` to find unfinished and planned tasks across all projects in a single call each.
3. If tasks exist in any scope, briefly summarise them grouped by project. Show in-progress tasks first, then planned.
