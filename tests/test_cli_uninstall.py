...
def test_uninstall_dispatches_single_agent() -> None:
    with (
        patch("sys.argv", ["llm-prompts", "uninstall", "kiro"]),
        patch("llm_prompts.install.uninstall") as mock_uninstall,
    ):
        main()
...
def test_uninstall_all_dispatches_none() -> None:
    with (
        patch("sys.argv", ["llm-prompts", "uninstall", "all"]),
        patch("llm_prompts.install.uninstall") as mock_uninstall,
    ):
        main()
...
def test_uninstall_passes_verbose_flag() -> None:
    with (
        patch("sys.argv", ["llm-prompts", "uninstall", "kiro", "--verbose"]),
        patch("llm_prompts.install.uninstall") as mock_uninstall,
    ):
        main()
